...
def computeMax(valuesToCompute, gtfsDB, city):
    count = 0
    max_values = {}  # Dictionary to store the maximum values for each field
    valuesToCompute = [x for x in valuesToCompute if x[0:4] != "time"]
    for point in gtfsDB['points'].find({'city': city}).sort([('city', 1)]):
        newPoint = point
        for field in valuesToCompute:
            max_value = float('-inf')  # Initialize to a very low value
            
            # Calculate the maximum value of the field
            for t in newPoint[field]:
                if t not in ['errStd', 'maxDiff', 'avg', 'max']:
                    if int(t) > 5 * 3600:
                        max_value = max(max_value, newPoint[field][t])
            
            # Store the maximum value in the point dictionary and the max_values dictionary
            max_value = max_value if max_value != float('-inf') else 0
            newPoint[field]['max'] = max_value
            max_values[field] = max(max_values.get(field, float('-inf')), max_value)

        # Update the database
        gtfsDB['points'].replace_one({'_id': point['_id']}, newPoint)
        print(f"\r {count} - {point['city']} - Max computed", end="\r")
        count += 1
